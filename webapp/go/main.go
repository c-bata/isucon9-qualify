...
func getSession(r *http.Request) *sessions.Session {
	defer measure.Start("get_session").Stop()
...
func getCSRFToken(r *http.Request) string {
	defer measure.Start("get_csrf_token").Stop()
...
func getUser(r *http.Request) (user User, errCode int, errMsg string) {
	defer measure.Start("get_user").Stop()
	session := getSession(r)
...
func getUserSimpleByID(q sqlx.Queryer, userID int64) (userSimple UserSimple, err error) {
	defer measure.Start("get_user_simple_by_id").Stop()
	user := User{}
...
func getPaymentServiceURL() string {
	defer measure.Start("get_payment_service_url").Stop()
...
func getShipmentServiceURL() string {
	defer measure.Start("get_shipment_service_url").Stop()
...
func getIndex(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("get_index").Stop()
	templates.ExecuteTemplate(w, "index.html", struct{}{})
}

func postInitialize(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_initialize").Stop()
	ri := reqInitialize{}
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := dbx.Select(&itemIDs,
			...
			"SELECT id FROM `items`, (SELECT item FROM `public_items` ORDER BY id DESC LIMIT ?) AS t WHERE t.item == items.id AND `status` IN (?,?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?))",
			ItemsPerPage+1,
			ItemStatusOnSale,
			ItemStatusSoldOut,
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_new_category_items").Stop()
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND category_id IN (?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?)) ORDER BY `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			time.Unix(createdAt, 0),
		...
		// 1st page
		inQuery, inArgs, err = sqlx.In(
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND category_id IN (?) ORDER BY id DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			ItemsPerPage+1,
...
func getUserItems(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_user_items").Stop()
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `seller_id` = ? AND `status` IN (?,?,?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?)) ORDER BY id` DESC LIMIT ?",
			userSimple.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
			ItemStatusSoldOut,
		...
		// 1st page
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `seller_id` = ? AND `status` IN (?,?,?) ORDER BY `id` DESC LIMIT ?",
			userSimple.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
			ItemStatusSoldOut,
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := tx.Select(&itemIDs,
			"SELECT id FROM `items` WHERE (`seller_id` = ? OR `buyer_id` = ?) AND `status` IN (?,?,?,?,?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?)) ORDER BY `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
		...
		// 1st page
		err := tx.Select(&itemIDs,
			"SELECT id FROM `items` WHERE (`seller_id` = ? OR `buyer_id` = ?) AND `status` IN (?,?,?,?,?) ORDER BY `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
...
func getItem(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_item").Stop()
...
func postItemEdit(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_item_edit").Stop()
	rie := reqItemEdit{}
...
func getQRCode(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("get_qr_code").Stop()
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_by").Stop()
	rb := reqBuy{}
...
func postShip(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_ship").Stop()
	reqps := reqPostShip{}
...
func postShipDone(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_ship_done").Stop()
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_complete").Stop()
...
func secureRandomStr(b int) string {
	defer measure.Start("secure_random_str").Stop()
	k := make([]byte, b)
...
func postBump(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_bump").Stop()
	rb := reqBump{}
	err := json.NewDecoder(r.Body).Decode(&rb)
	if err != nil {
		outputErrorMsg(w, http.StatusBadRequest, "json decode error")
...
func getSettings(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("get_settings").Stop()
...
func postLogin(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_login").Stop()
	rl := reqLogin{}
...
func postRegister(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("post_register").Stop()
	rr := reqRegister{}
...
func getReports(w http.ResponseWriter, r *http.Request) {
	defer measure.Start("get_reports").Stop()
...
func outputErrorMsg(w http.ResponseWriter, status int, msg string) {
	defer measure.Start("output_error_msg").Stop()
	w.Header().Set("Content-Type", "application/json;charset=utf-8")
...
func getImageURL(imageName string) string {
	defer measure.Start("get_image_url").Stop()
